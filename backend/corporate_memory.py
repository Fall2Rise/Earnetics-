...
class CorporateMemory:
    ...
    def _ensure_task_columns(self, cursor: sqlite3.Cursor) -> None:
        cursor.execute("PRAGMA table_info(department_tasks)")
        existing = {row[1] for row in cursor.fetchall()}
        if "claimed_by" not in existing:
            cursor.execute("ALTER TABLE department_tasks ADD COLUMN claimed_by TEXT")
        if "claimed_at" not in existing:
            cursor.execute("ALTER TABLE department_tasks ADD COLUMN claimed_at TEXT")
        if "sla_minutes" not in existing:
            cursor.execute(
                "ALTER TABLE department_tasks ADD COLUMN sla_minutes INTEGER DEFAULT 0"
            )
...
def get_latest_earnetics_vision(
    ...
    cur.execute(
        """
        SELECT id, title, directive_type, owner, status, payload, due_date,
               created_at, updated_at
        FROM executive_directives
        WHERE directive_type = 'earnetics_vision'
          AND (status IS NULL OR status != 'archived')
