...
export type IntegrationsResponse = Record<string, IntegrationStatus>;

export const fetchIntegrations = async (signal?: AbortSignal): Promise<IntegrationsResponse> => {
  const response = await fetch(`${API_BASE_URL}/api/system/integrations`, {
    signal,
    headers: getAuthHeaders(),
  });

  if (!response.ok) {
    throw new Error(`Unable to load integrations (${response.status})`);
  }
