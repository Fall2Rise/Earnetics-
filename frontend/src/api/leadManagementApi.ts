...
export const leadManagementApi = {
  ...
  async addLeadToList(leadId: number): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/api/leads/scraped/${leadId}/add-to-list`, {
      method: 'POST',
      headers: {
        'X-Fallat-Token': localStorage.getItem('api_token') || '',
      },
    });
    if (!response.ok) throw new Error('Failed to add lead to list');
    return response.json();
  },
