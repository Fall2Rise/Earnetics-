    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-black overflow-hidden">
      ...
      <div className="absolute top-6 left-6 z-10 pointer-events-none">
        <motion.div
          ...
          className="bg-black/70 backdrop-blur-xl border-2 border-cyan-500/40 rounded-xl p-5 min-w-[320px] shadow-2xl shadow-cyan-500/20 pointer-events-auto"
        >
          ...
          <button 
            ...
            className="w-full mt-4 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/50 rounded-lg py-2 font-bold transition-all flex items-center justify-center gap-2 group"
          >
        ...
        <motion.div
          ...
          className="mt-4 bg-black/70 backdrop-blur-xl border-2 border-cyan-500/40 rounded-xl p-4 max-h-[400px] overflow-y-auto shadow-2xl shadow-cyan-500/20 pointer-events-auto"
        >
          ...
          <div className="space-y-2">
            {DEPARTMENT_ZONES.map((zone) => {
              ...
              return (
                <div
                  key={zone.department}
                  className="flex items-center justify-between text-xs p-2 rounded hover:bg-white/5 transition-colors cursor-pointer"
                  onClick={() => {
                    setSelectedDepartment(selectedDepartment === zone.department ? null : zone.department);
                    if (selectedDepartment !== zone.department) {
                      setTargetAgent({ ...agents[0], department: zone.department } as Agent);
                    } else {
                      setTargetAgent(null);
                    }
                  }}
      ...
      <AnimatePresence>
        {missionControlOpen && (
          <MissionControlPanel onClose={() => setMissionControlOpen(false)} />
        )}
      </AnimatePresence>
