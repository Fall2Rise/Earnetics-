...
export const QuantumSuccessionPanel: React.FC = () => {
  ...
  const handleLog = async () => {
    ...
    try {
      await logAuditReason({
        action: 'quantum_succession_request',
        reason,
        directive_ref: 'dynastic_succession_matrix',
        risk_level: 'RED',
        owner_approved: approval,
        cryptographic_approval: cryptoApproval,
      });
      setStatus('Request logged. Awaiting succession gate.');
      setReason('');
      setApproval(false);
  ...
  return (
    <div className="glass-panel p-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl text-white">Quantum Succession</h3>
          <p className="text-sm text-slate-300">Ownership transfer protocol (RED tier).</p>
        </div>
      ...
      <div className="mt-6 grid gap-3">
        <input
          ...
          placeholder="Succession reason / trigger"
          value={reason}
