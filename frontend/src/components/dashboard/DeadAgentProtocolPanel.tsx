...
export const DeadAgentProtocolPanel: React.FC = () => {
  ...
  const handleLog = async () => {
    ...
    try {
      await logAuditReason({
        action: 'dead_agent_protocol_request',
        reason,
        directive_ref: 'succession_and_continuity',
        risk_level: 'RED',
        owner_approved: approval,
        cryptographic_approval: cryptoApproval,
      });
      setStatus('Request logged. Awaiting approval gate.');
      setReason('');
      setApproval(false);
  ...
  return (
    <div className="glass-panel p-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl text-white">Dead Agent Protocol</h3>
          <p className="text-sm text-slate-300">Fallback and shutdown policy (RED tier).</p>
        </div>
      ...
      <div className="mt-6 grid gap-3">
        <input
          ...
          placeholder="Reason / incident summary"
          value={reason}
