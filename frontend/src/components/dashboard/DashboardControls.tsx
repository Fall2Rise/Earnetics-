...
export const DashboardControls: React.FC = () => {
  ...
  const [cycleMessage, setCycleMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  ...
  const handleRunCycle = useCallback(async () => {
    try {
      ...
      const result = await runAutonomousCycle('Manual dashboard trigger', { source: 'dashboard_controls' });
      
      if (result.status === 'error') {
        throw new Error(result.message || 'Cycle failed');
      }

      const pending = result.pending as number | undefined;
      const workerId = result.worker_id as string | undefined;
      const msg = pending !== undefined 
        ? `Cycle triggered (Worker: ${workerId}, Queue: ${pending})` 
        : 'Cycle triggered successfully';
      setCycleMessage({ type: 'success', text: msg });
      
      // Dispatch event to refresh operations pulse immediately
      window.dispatchEvent(new CustomEvent('refresh-operations'));
      ...
      setCycleMessage({ type: 'error', text: err instanceof Error ? err.message : 'Cycle failed' });
    } finally {
      ...
      <div className="controls-grid">
        ...
        <div className="control-item control-item--action">
          <div className="control-label">
            ...
            {cycleMessage && (
              <div className={`text-xs mt-1 ${cycleMessage.type === 'error' ? 'text-red-400' : 'text-green-400'}`}>
                {cycleMessage.text}
              </div>
            )}
          </div>
