  ...
  const handleCreate = async () => {
    ...
    try {
      await logAuditReason({
        action: 'knowledge_vault_entry',
        reason,
        directive_ref: 'truth_seeking_and_clarity',
        risk_level: 'GREEN',
      });
  ...
  return (
    <div className="glass-panel p-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl text-white">Knowledge Vault</h3>
          <p className="text-sm text-slate-300">Verified knowledge assets and research artifacts.</p>
        </div>
      </header>
      ...
      <div className="mt-6 space-y-3">
        {items.length === 0 ? (
          <p className="text-sm text-slate-300">No knowledge assets found.</p>
        ) : (
          items.map((item) => (
