...
export const Office3DView: React.FC = () => {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[Office3DView] System status sync failed:', err);
      // Don't set blocking error, just log it. 
      // The UI handles null status gracefully (shows 0 revenue/unknown status)
    }

    // 2. Always try to fetch agents (store handles its own mock fallback)
    try {
      await fetchAgents();
    } catch (err) {
      console.error('[Office3DView] Agent fetch failed:', err);
      // Only set error if agents fail completely (which shouldn't happen with mock fallback)
      if (mountedRef.current) {
