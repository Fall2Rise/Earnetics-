...
export const Web3DepartmentPanel: React.FC = () => {
  ...
  const handleLog = async () => {
    ...
    try {
      await logAuditReason({
        action: 'web3_department_request',
        reason,
        directive_ref: 'security_and_integrity',
        risk_level: 'RED',
        owner_approved: approval,
        cryptographic_approval: cryptoApproval,
      });
      setStatus('Request logged. Web3 features remain locked.');
      setReason('');
      setApproval(false);
  ...
  return (
    <div className="glass-panel p-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl text-white">Web3 Department</h3>
          <p className="text-sm text-slate-300">Wallets, contracts, DAO ops (locked by default).</p>
        </div>
